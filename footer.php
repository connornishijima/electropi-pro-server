...
function updateHues(lampList,color){
	hueTTime = <?php echo intval(floatval($sets["SETTINGS"]["hueTTime"])*1000);?>;
	document.getElementById("HUE-"+lampList).value = color;
	$("#HUEB-"+lampList).animate({backgroundColor: color }, hueTTime);
	lampList = lampList.split("_");
	lampList.forEach(function(entry){
		document.getElementById("HUE-"+entry).value = color;
		$("#HUEB-"+entry).animate({backgroundColor: color }, hueTTime);
	});
}
